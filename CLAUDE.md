...
# Install shared Python package first (required before any Python service)
...
Python services use `asyncio_mode = "auto"` — no `@pytest.mark.asyncio` needed.
...
Data: MongoDB :27017 (documents, vectors, configs, users)
      Redis :6379 (Celery broker, task results, cache)
```

- **Gateway** — JWT validation, CORS, rate limiting, reverse proxy, SSE/WebSocket passthrough. Auth endpoints (`/api/v1/auth/*`) skip JWT check; all others require it.
- **Config Service** — User auth (register/login/refresh), config CRUD, folder browsing/scanning, YAML export/import.
- **Ingestion Service** — Document processing pipeline: processors → chunkers → embedders → storage. Celery workers run async. Supports PDF, DOCX, TXT, MD, HTML, images (OCR).
- **RAG Service** — 6 LangGraph agent types (Naive, ReAct, CRAG, Self-RAG, Multi-Query, Plan-Solve), 4 retrieval strategies (vector, keyword, graph, hybrid), 4 LLM providers (OpenAI, Anthropic, Ollama, vLLM). SSE streaming via sse-starlette.

Frontend apps live in `apps/` (NOT `ui/` — that's a legacy placeholder with only .gitkeep files):
- `apps/configurator-ui` — Vue 3 + Tailwind + Headless UI wizard for pipeline configuration
- `apps/sandbox-ui` — Vue 3 + Tailwind + Naive UI chat interface for testing pipelines

## Key Patterns

- **Shared models**: Import from `rag_config_common` (installed from `shared/python/`). Use these for all config/user models and enums — never redefine them in services.
...
- **API response envelope**: All endpoints return `{"success": bool, "data": ..., "error": ..., "meta": ...}` via the shared `APIResponse` wrapper.
